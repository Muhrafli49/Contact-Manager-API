...
import { createTestUser, removeAllTestContacts, removeTestUser, createTestContact, getTestContact, createManyTestContact } from "./test-util"
    ...
    it('should reject if request is not valid', async () => {
        const result = await supertest(web)
            ...
            .send({
                ...
                lastName: "test",
                email: "test",
                phone: "1234567890324444444444412312312",
            });
...
describe('PUT /api/contacts/:contactsId', () => {
    ...
    it('should can update existing contact', async () => {
        ...
        expect(result.status).toBe(200);
        expect(result.body.data.id).toBe(testContact.id)
        expect(result.body.data.firstName).toBe("Muh");
        expect(result.body.data.lastName).toBe("Rapl");
        expect(result.body.data.email).toBe("test2@example.com");
        expect(result.body.data.phone).toBe("082351261362");
    });
